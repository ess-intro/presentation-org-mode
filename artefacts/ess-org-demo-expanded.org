# (if all you see is the next line, ending in =...=, then the buffer
# is "folded"; to see the rest of the buffer put your cursor at the
# beginning of the next line and type [TAB] twice.
...
the same command, =[M-x org-babel-execute-src-block]= (=[C-c C-c]=)
(with point inside the source block):
...
To enabling editing R code (if you get an error message /No such
language mode: R-mode/), you may need to evaluate the following code:
...
Here the code creates a graphic file (named by the =:file= header
argument).

#+begin_src R :file speed.png :results output file graphics :exports code
  ## from "?graphics::plot"
  Speed <- cars$speed
  Distance <- cars$dist
...
Mode.  DT ("Distro Tube"?) has yet another [[https://www.youtube.com/watch?v=Ea_-TaEGa7Y][introductory video]].  And,
tdhock has a playlist [[https://www.youtube.com/playlist?list=PLwc48KSH3D1Onsed66FPLywMSIQmAhUYJ][Emacs and R]].
