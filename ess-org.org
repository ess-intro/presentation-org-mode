...
#+begin_src elisp :results none
  (defun orgify-all-non-org-blocks ()
    ;; take every non-org block and insert it between #+begin_src
    ;; org...#+end_src
    (let ((to-insert "#+begin_src org :exports code :noweb nil\n#+end_src\n")
          (pt (point))
          (filtered (org-filter-by-argument-value
                      ':exports
                      (list "results" "both")
             ...
             (lambda (l)
               ...
               (let* ((context (org-element-context))
                      ...
                      (results-end (if results-begin
                                     ...
                                     nil))
                      (end (if results-end results-end src-end))
                      (contents (delete-and-extract-region src-begin end)))
                 (message "delete-and-extract region %s %s: %s" src-begin end contents)
                 (insert to-insert)
                 (goto-char l)            ; go back to beginning of new src block
