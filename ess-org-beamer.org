...
To evaluate an entire source block in an Org buffer, where the major
mode will be that of the language, type =[C-c C-c]= or =[C-c C-v e]=.
...
- the Org Mode [[https://orgmode.org/worg/][worg]] site ("=... a collectively-built knowledge
  database with lots of Org-Mode Resources.=")
