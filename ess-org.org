...
=[[properties][:PROPERTIES:]]= holding, well, properties, i.e, (key,
value) tuples.
...
Org Mode has nice support for tables, e.g.,
...
Org Mode provides facilities for defining equations to calculate cell
values, etc.
...
Org Mode buffers can be exported to HTML, PDF, and many other formats.
...
available in Emacs (or, using =info(1)=).
...
out of the .org buffer"?), normally with =[C-c ']=.  The Org Src
blocks typically have better support for Emacs so-called "font locks",
which do program language-specific source code highlighting, etc.
...
A source block can be evaulated in one of two sorts of contexts,
...
When processing a [[tables][table]] result of a previous source block to be used as
an input [[variables][variable]] to another code block, the =:colnames= header
