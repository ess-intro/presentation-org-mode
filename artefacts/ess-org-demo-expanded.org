...
#+begin_quote
First, some notation :: A string inside square brackets, =[abecde]=,
means you, the user, should type the string, leaving out the square
brackets.  Also, the Emacs convention for control and modifier (alt)
keys is used.  So, =[C-x b]= means type =control x=, then the letter
=b=.  And, =[M-x b]= would mean type =modifier x= (maybe better known
as =alt x=), then the letter =b= (almost certainly an error!).
#+end_quote

(To skip the introductory material and go straight to the
demonstration work, click [[pastintro][here]].)
...
You should be able to find that file
<a href="https://ess-intro.github.io/presentation-org-mode/artefacts/ess-org-demo-expanded.org">here</a>
(<em>This</em> file will not work as a .org file.)
...
command =[M-x org-insert-structure-template]= (=[C-c C-,]=), and
entering the type of block you are interested in creating, e.g.,
...
=[M-x org-insert-structure-template]= (=[C-c C-,]=) and then one of
the following
- =a=
- =c=
- =C=
...
=[M-x org-insert-structure-template]= (=[C-c C-,]=) and entering
[=s=].)
...
or =#+end_src= line), and type =[M-x org-babel-execute-src-block]=
(=[C-c C-c]=) (not the square brackets; just what is inside).
Additionally, evaluating code in a file can be a security risk, so you
will be prompted to make sure you want to execute the code.
...
An R source block, for example, can be evaluated using the same
command, =[M-x org-babel-execute-src-block]= (=[C-c C-c]=):
...
trigger the triggering, you will need to type
=[M-x org-babel-execute-src-block]= (=[C-c C-c]=) with point
...
=M-x org-export-dispatch= (=[C-c C-e]=).  Org mode then pops up a
window that allows you to customize your export.  At the top of this
window are mostly options for defining the scope of the export.  For
example, if you then type =[C-s]=, only the subtree in which the
export is initialized will be exported.
...
If you type =M-x org-export-dispatch= (=[C-c C-e]=), then [C-s] (for
"subtree"), then [h] (for "html"), and [o] (for "open"), then,
assuming your Emacs is configured to know how to open a .html file in
a browser (90% likely, I'd guess), you'll see the text in this
subtree, the above code, and the indices of the valleys (the last
result).
...
Now, you could, if you chose, enter =[M-x org-babel-tangle]=
(=[C-c C-v t]=).  But, be warned!  This will tangle all the source
blocks in the current (this) .org file that have a =:tangle= header
argument (specified or inherited).  One can restrict the tangle
operation to the source block at point (see =[M-x describe-function]=
(=[C-h f]=) for =org-babel-tangle= for details), but even so, I would
...
- =[M-x org-edit-src-abort]= (=[C-c C-k]=) :: to abort the current editing, discarding any
  unsaved changes
- =[M-x org-edit-src-exit]= (=[C-c ']=) :: to save any unsaved changes back to the
  .org buffer and close the source edit buffer

NB: If you are editing a block that has [[noweb][:noweb]] references,
the references will *not* be expanded when passing the block (in part
or in whole) to a running iESS instance (via, e.g.,
=[M-x org-babel-execute-src-block]= (=[C-c C-c]=) or
=[M-x ess-eval-buffer]= (=[C-c C-b]=)).  And, in fact, the R
interpreter will likely flag a syntax error should you try to do so.
...
During a given Emacs instance, the first time you type
=[M-x org-babel-execute-src-block]= (=[C-c C-c]=) in a source block
with a given =:session= name, Emacs will prompt you for the name of
the starting directory (with a default being the directory of the
...
After evaluations, you can use =[M-x switch-to-buffer]= (=[C-x b]=) to
switch to that buffer and inspect the state, maybe set debug
breakpoints, or debug after error or debug statement (=browser()=,
say) in your code.
...
=[M-x org-babel-execute-src-block]= (=[C-c C-c]=), it will sleep the
two seconds.  However, after that, evaluating it will return results
immediately.
...
The beamer slides for this tutorial are [[https://ess-intro.github.io/presentation-org-mode/artefacts/ess-org-beamer.html][here]] ([[https://ess-intro.github.io/presentation-org-mode/artefacts/ess-org-beamer.pdf][pdf]]).  A .html version
