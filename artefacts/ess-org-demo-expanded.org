...
command =[M-x org-insert-structure-template]= (=[C-c C-,]=), and entering the type of block you are
...
=[M-x org-insert-structure-template]= (=[C-c C-,]=) and then one of the following
- =a=
- =c=
- =C=
- =E=
...
=[M-x org-insert-structure-template]= (=[C-c C-,]=) and entering [=s=].)
...
to =yes= to enable noweb processing.  Again, as with all header
